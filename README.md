...
**3. 전체 코드 흐름 + Reflection 심화 (10분)**

- Reflection 기본 구조: 90일마다 gpt-4.1이 자기평가 → 다음 30일 계획에 반영
- 구현 과정에서 겪은 문제들:
    - 1차: 성찰 내용이 추상적 → 매번 "성과를 올려라" 일반론 반복, 효과 없음
    - 2차: 승진 요건 갭 수치를 프롬프트에 제공 → 구체성은 살렸지만 mini 모델이 무시
    - 3차: 프롬프트 강화 (처방 형식, 강제 지시) → 여전히 LLM 판단에 의존하는 한계
- 핵심 교훈: **Reflection 결과를 텍스트로만 전달하면 의사결정 모델이 무시할 수 있다** → 구조적으로 행동에 영향을 미치는 설계가 필요
...
- 프로젝트 목적 : 딥 에이전트 기능 심화 학습 (Reflection)
    - 단순히 LLM에게 질문하고 답을 받는 게 아니라, LLM이 직접 **의사결정 주체**가 되어
    주어진 기간동안 자율적으로 행동하고, 그 결과를 관찰하는 구조
- 5가지 성향 비교 → **어떤 성향이 회사에서 가장 오래, 가장 높이 올라갈까?**
...
- Reflection(자기성찰) on/off 비교
    ...
    - 이게 실제로 효과가 있는지, 오히려 방해가 되는지 같은 성향끼리 비교 실험
...
1. 성과형 (임원) : 연봉 3억 / 20년 / 현직유지
    - 업무 능력과 성과에 올인하는 전략
    - 승진 요건을 가장 빠르게 충족시켜 승진이 빠름
2. 사교형 (임원) : 연봉 2.8억 / 20년 / 현직유지
    - 동료 관계와 평판 중심의 전략
    - 평판이 올라가면 승진 요건 중 하나를 안정적으로 충족, 동료 관계가 좋으면 이벤트에서도 유리한 효과
3. 균형형 (이사) : 연봉 1.8억 / 20년 / 정년퇴직
    - 모든 스탯을 골고루 올리니까 어디서도 크게 밀리지 않지만, 반대로 어느 한 스탯이 확 튀지도 않아서 임원까지는 못 감
4. 워라밸형 (과장) : 연봉 8,500만 / 15년 / 권고사직
    - 해고 시점에 승진에 필요한 스탯을 전부 충족하지만, 승진 확률 판정에서 3번 탈락하면서 권고사직 처리
    - 스트레스 관리에 집중하느라 스탯 여유분이 크지 않았고, 승진 심사에서 운이 안따름
5. 정치형 (사원) : 연봉 4,100만 / 5년 / 권고사직
    - 상사 라인을 타고 정치력을 올리는데, "정치적으로 행동한다" 행동이 평판을 깎음
    - 상사신뢰와 정치력은 올라가지만 평판이 바닥을 치니까, 대리 승진에 필요한 평판 12를 못 채움

### 그래프 해석

- 성과형 : 업무능력과 성과가 빠르게 상승
- 사교형 : 동료관계과 평판이 꾸준히 높음
- 워라밸형 : 스트레스가 낮고 체력이 높은 대신, 다른 스탯들의 상승 속도가 느림
- 정치형 : 상사신뢰는 높은데 평판이 바닥

## Reflection(자기성찰) on/off 비교 결과

> 가장 빨리 해고되는 **정치형**에 대해서 자기성찰을 할 경우 얼마나 변화가 있을지 실험

1. Reflection ON : 부장 / 연봉 1.3억 / 17년 / 희망퇴직
    - 해고가 아니라 희망퇴직, 자발적으로 퇴직
2. Reflection OFF : 사원 / 연봉 4,100만 / 5년 / 권고사직
    - 성향별 비교 결과와 마찬가지로 평판이 바닥이라 승진 못함

→ 차이가 극명함

### 핵심

일정 기간(90일)마다 자기 상태를 점검

→ Reflection 모델(gpt-4.1)이 현재 스탯과 승진 요건 사이의 갭을 분석하고, "평판이 12 필요한데 지금 3밖에 안 된다, 정치 행동을 줄이고 동료를 도와서 평판을 올려라"라는 식의 처방을 내림

→ 이 처방이 다음 30일 행동 계획을 짜는 LLM(gpt-4.1-mini)의 프롬프트에 들어감

→ 결과적으로 Reflection이 있는 에이전트는 **정치 행동 일변도에서 벗어나서 평판을 보완**할 수 있었고, 대리 승진을 넘긴 이후로는 꾸준히 올라감

반면, Reflection OFF인 에이전트는 정치 성향대로 정치적인 행동만 반복하다가 평판이 바닥을 기어서 5년차에 짤림
...
- Reflect는 17년차에 희망퇴직 : 스트레스 37, 체력 80 상태에서 자발적으로 퇴직 거라, 나쁜 결말이 아님
